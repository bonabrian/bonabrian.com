...
const CodingActivity = () => {
  ...
  const startDate = data?.start ? formatDate(data?.start) : ''
  const endDate = data?.end ? formatDate(data?.end) : ''
  const dailyAverage =
  ...
  return (
    <div className={cn('flex flex-col gap-y-2 bg-gradie')}>
      ...
      <div className={cn('mb-1 grid md:grid-cols-2 gap-3 py-2')}>
        ...
        <OverviewCard label="All Time Since Today" value={allTimeSinceToday} />
      </div>
